...
The CTF server's IP address is [`34.65.72.129`](http://34.65.72.129/). You can see a leaderboard there and it is the address for submitting answers. The first way we'll look at submitting answers is with `curl` in Linux.
...
```bash
curl 'http://34.65.72.129/user/d.wilson'
```
...
```bash
curl 'http://34.65.72.129/answers/'
```
...
data = {"number": "1",
        "answer": "",
        "user": "d.wilson"}
r = requests.post("http://34.65.72.129/", data=data)
```
