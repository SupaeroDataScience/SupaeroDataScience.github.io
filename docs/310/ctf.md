...
The CTF server's IP address is [`34.65.220.146`](http://34.65.220.146/). You can see a leaderboard there and it is the address for submitting answers. The first way we'll look at submitting answers is with `curl` in Linux.
...
```bash
curl -X POST 'http://34.65.220.146/' \
    -d 'number=1' \
...
```bash
curl 'http://34.65.220.146/user/d.wilson'
```
...
```bash
curl 'http://34.65.220.146/answers/'
```
...
data = {"number": "1",
        "answer": "",
        "user": "d.wilson"}
r = requests.post("http://34.65.220.146/", data=data)
```
...
Your user on the server is `ctf` and the IP is: `34.65.220.146`.
